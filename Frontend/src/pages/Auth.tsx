...
import { Sparkles, Eye, EyeOff, ArrowLeft, Mail, Lock, User, Github, Info, Timer, RefreshCw } from "lucide-react";
...
const Auth = () => {
  ...
  // Track where we came from to handle Resend correctly
  ...
  // Reset timer when entering OTP mode
  useEffect(() => {
    if (mode === "otp") {
      setTimeLeft(60);
      setCanResend(false);
  ...
  const handleResendOtp = async () => {
    ...
    try {
      let result;
      // Resend based on previous context
      if (previousMode === "signup" || name) {
        // Assuming signup flow if name is present
        result = await signup(email, password, name);
      } else {
        // Default fallback or login/admin flow
      ...
      if (result.success) {
        ...
        toast({
          title: "Resend Failed",
          description: result.error || "Could not resend code",
          variant: "destructive"
        });
      }
    } catch (e) {
      toast({
        title: "Error",
        description: "Failed to resend code due to network error",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        // 1. Check for Admin OTP Flow (First Priority)
        if (result.success && (result as any).requiresOTP && (result as any).isAdminFlow) {
          toast({
            title: "God Mode Credentials Detected",
            description: "Enter the security code to access the Admin Portal.",
        ...
        // Use string OTP directly
        const result = await verifyOTP(email, otp);

        if (result.success) {
          // 3. Handle Admin Redirect after OTP
